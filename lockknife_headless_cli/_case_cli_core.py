...
def register(case_group: Any, cli: Any) -> None:
    ...
    @click.option(
        "--output", type=click.Path(file_okay=False, path_type=pathlib.Path), required=True
    )
    def init_cmd(
        case_id: str,
        examiner: str,
    ...
    def manifest_cmd(case_dir: pathlib.Path, out_format: str) -> None:
        ...
        if out_format.lower() == "json":
            ...
            return
        cli.console.print(
            f"{manifest['case_id']} | artifacts={len(manifest['artifacts'])} | examiner={manifest['examiner']}"
        )
    ...
    def export_cmd(
        ...
        include_registered_artifacts: bool,
        ...
        if output is None:
            output = cli.case_output_path(
                case_dir, area="exports", filename=f"{manifest.case_id}_bundle.zip"
            )
